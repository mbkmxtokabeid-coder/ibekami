<div class="bg-[#fff2e0] min-h-screen font-sans text-[#3d2b1f]">
    ...
    <section class="relative bg-[#fff2e0] overflow-hidden border-b border-[#ff9100]/10">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-24 relative z-10 text-center">
            ...
            <h1 class="font-['Playfair_Display'] text-4xl md:text-6xl font-bold text-[#3d2b1f] leading-tight mb-4">
                {{ __('messages.best_quality') }} <em class="italic text-[#ff9100] not-italic">{{ __('messages.for_you') }}</em> {{ __('messages.for_you_text') }}
            </h1>
        </div>
    </section>
    ...
    <main class="max-w-7xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-16 items-start py-16">
        ...
        <section x-data="{ activeImg: '{{ $productData['images'][0] ?? $productData['image'] }}' }">
            ...
            <div class="flex gap-4 mt-6">
                ...
                <button 
                    ...
                    class="thumbnail-btn w-20 h-20 rounded-[20px] overflow-hidden border-2 transition-all shadow-sm {{ $index === 0 ? 'border-[#ff9100] scale-105' : 'border-transparent opacity-60' }}" 
                    :class="activeImg === '{{ $image }}' ? 'border-[#ff9100] scale-105' : 'border-transparent opacity-60'">
        ...
        <section class="flex flex-col space-y-8">
            <div>
                ...
                <div class="mt-4 flex items-center gap-3">
                    @if($productData['discount'] > 0)
                        ...
                        @endphp
                        <span class="text-2xl font-bold text-[#ff9100]">Rp {{ number_format($discountedPrice, 0, ',', '.') }}</span>
                    ...
                    @else
                        <span class="text-2xl font-bold text-[#ff9100]">Rp {{ number_format($productData['price'], 0, ',', '.') }}</span>
                    @endif
                </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-[32px] p-8 shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-[#ff9100]/5">
                <h2 class="text-2xl font-bold text-[#3d2b1f] mb-6">{{ __('messages.product_details') }}</h2>
                ...
                <div class="space-y-4">
                    @if(!empty($productData['details']))
                        @foreach($productData['details'] as $key => $value)
                            <div class="grid grid-cols-[120px_20px_1fr] text-[15px] md:text-lg font-medium text-[#3d2b1f]">
                                <span class="text-[#3d2b1f]">{{ ucfirst($key) }}</span>
                    ...
                    @else
                        <div class="grid grid-cols-[120px_20px_1fr] text-[15px] md:text-lg font-medium text-[#3d2b1f]">
                            <span class="text-[#3d2b1f]">{{ __('messages.category') }}</span>
                            <span class="text-center">:</span>
                            <span class="font-normal">{{ $productData['category'] }}</span>
                        </div>
                        <div class="grid grid-cols-[120px_20px_1fr] text-[15px] md:text-lg font-medium text-[#3d2b1f]">
                            <span class="text-[#3d2b1f]">{{ __('messages.status') }}</span>
                            <span class="text-center">:</span>
            ...
            <div class="flex flex-col sm:flex-row gap-4 w-full">
                ...
                <a href="https://wa.me/628170769999?text=Halo%20Admin%2C%20saya%20tertarik%20dengan%20produk%20dari%20Ibekami.id.%20Bisa%20bantu%20untuk%20info%20lebih%20lanjut%3F" 
                   ...
                   class="flex-[2] bg-[#ff9100] hover:bg-[#e68200] text-white py-5 rounded-[24px] flex items-center justify-center gap-4 font-black text-lg transition-all shadow-[0_15px_30px_rgba(255,145,0,0.3)] active:scale-[0.98]">
    ...
    <section class="max-w-7xl mx-auto px-6 py-24 border-t border-[#ff9100]/10">
        <h2 class="font-['Playfair_Display'] text-3xl font-bold text-[#3d2b1f] mb-12">{{ __('messages.you_may_also_like') }}</h2>
